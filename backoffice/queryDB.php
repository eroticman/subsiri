...
if ( $_POST['action'] == 'delUser' ) {
	$sql = "UPDATE db_admin SET is_active = '0' WHERE id = {$_POST['uid']}";
	$result = query($sql);
...
if ( $_POST['action'] == 'delCustomer' ) {
	$sql = "UPDATE db_customer SET is_active = '0' WHERE id = {$_POST['cid']}";
	$result = query($sql);
...
if ( $_POST['action'] == 'delCost' ) {
	$sql = "UPDATE db_cost SET is_active = '0' WHERE cost_id = {$_POST['cid']}";
	$result = query($sql);
...
if ( $_POST['action'] == 'delBooking' ) {
	$sql = "UPDATE db_booking SET is_active = '0' WHERE booking_id = {$_POST['bid']}";
	$result = query($sql);

	$sql = "UPDATE db_product SET is_status = '1' WHERE id = {$_POST['pid']}";
	$result = query($sql);
